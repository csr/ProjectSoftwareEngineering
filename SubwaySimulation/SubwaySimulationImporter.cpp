...
void openFile(string fileName) {
  ...
  if (!doc.LoadFile ("testInput/example.xml")) {
    cerr << doc.ErrorDesc() << std::endl;
  } else {
    cout << "Ok, file found!" << std::endl;
  }
}
...
int findRoot(TiXmlDocument doc){
  return 0;
  // cesare: commentato perchè non compila
...
// cesare: questo controllo lo farei nella classe Subway
// sui dizionari/array/vettori che contengono i dati (tram e stazioni)
